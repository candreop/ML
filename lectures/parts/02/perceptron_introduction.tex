...
\begin{frame}[t]{Perceptron loss function}
    ...
    of the Mark 1 \index{perceptron}\gls{perceptron},
    ...
    does the \index{perceptron}\gls{perceptron} optimize?\\
...
\begin{frame}[t]{Perceptron loss function}
    ...
    \vspace{0.3cm}
    The \index{loss function}\gls{loss function}, $L$,
    can be expressed in a form that resembles 
    \index{least-squares}\gls{least squares} 
    \index{regression}\gls{regression}\footnote{
...
\begin{frame}[t]{Perceptron loss function}
    ...
    %     \nabla_{\vect{w}} L = 
    %         \nabla_{\vect{w}} \Bigg\{ 
    %           \sum_{(\vect{x},y) \in \mathcal{D}} \Bigg(y - sign\Big\{ \vect{w}^T \cdot \vect{x} \Big\}\Bigg)^2
    %         \Bigg\} 
    %  \end{equation}

    Unlike in \index{least-squares}\gls{least squares} \index{regression}\gls{regression},
    where the gradient of the \index{loss function}\gls{loss function} is a smooth and continuous function, 
    here we have to deal with the gradient of the {\em sign} function. \\
    \vspace{0.3cm}
    The \index{loss function}\gls{loss function} of Eq.~\ref{eq:percepton_leastsquares_loss_2}
